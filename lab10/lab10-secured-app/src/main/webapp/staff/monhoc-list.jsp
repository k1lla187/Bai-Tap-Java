...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .search-box input { padding: 10px 14px; border: 2px solid #e0e0e0; border-radius: 8px; width: 300px; }
...
<body>
    <nav class="navbar">
        <div class="navbar-brand">Quan ly Mon Hoc</div>
    ...
    <div class="container">
        <div class="page-header">
            ...
            <a href="${pageContext.request.contextPath}/staff/monhoc-form.jsp" class="btn btn-primary">+ Them Mon Hoc</a>
        </div>
        <c:if test="${not empty error}"><div class="error">${error}</div></c:if>
        <div class="search-box">
            <form method="get" action="${pageContext.request.contextPath}/staff/monhoc">
                <input type="text" name="keyword" placeholder="Tim kiem ma mon, ten mon..." value="${keyword}">
                <button type="submit" class="btn btn-primary" style="margin-left:8px;">Tim kiem</button>
                <a href="${pageContext.request.contextPath}/staff/monhoc" class="btn btn-secondary">Xoa</a>
            </form>
        </div>
        <table>
            <thead><tr><th>ID</th><th>Ma mon</th><th>Ten mon</th><th>So tin chi</th><th>Hanh dong</th></tr></thead>
            <tbody>
                <c:forEach var="m" items="${dsMonHoc}">
                    <tr>
                        <td>${m.id}</td><td>${m.maMon}</td><td>${m.tenMon}</td><td>${m.soTinChi}</td>
                        <td class="actions">
                            <a href="${pageContext.request.contextPath}/staff/monhoc?action=edit&id=${m.id}" class="edit">Sua</a>
                            <a href="${pageContext.request.contextPath}/staff/monhoc?action=delete&id=${m.id}" class="delete" onclick="return confirm('Xoa mon hoc nay?')">Xoa</a>
                        </td>
                    </tr>
                </c:forEach>
                <c:if test="${empty dsMonHoc}"><tr><td colspan="5" style="text-align:center;color:#999;padding:30px;">Khong co mon hoc nao</td></tr></c:if>
            </tbody>
        </table>
    </div>
</body>
