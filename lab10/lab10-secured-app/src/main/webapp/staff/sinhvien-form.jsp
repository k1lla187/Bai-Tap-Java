...
<html>
<head>
    <title>${empty sinhVien ? 'Them' : 'Sua'} Sinh Vien</title>
    <style>
...
<body>
    <div class="top-bar">
        <span class="brand">${empty sinhVien ? 'Them' : 'Sua'} Sinh Vien</span>
        <a href="${pageContext.request.contextPath}/staff/sinhvien">&larr; Quay ve danh sach</a>
    </div>
    <div class="wrapper">
        <div class="form-card">
            <h2>${empty sinhVien ? 'Them sinh vien moi' : 'Sua thong tin sinh vien'}</h2>
            <c:if test="${not empty error}"><div class="error">${error}</div></c:if>
            <form method="post" action="${pageContext.request.contextPath}/staff/sinhvien">
                <c:if test="${not empty sinhVien}"><input type="hidden" name="id" value="${sinhVien.id}"></c:if>
                <div class="form-group">
                    <label>Ma Sinh Vien *</label>
                    <input type="text" name="maSV" value="${sinhVien.maSV}" required placeholder="VD: SV001">
                </div>
                <div class="form-group">
                    <label>Ho ten *</label>
                    <input type="text" name="hoTen" value="${sinhVien.hoTen}" required placeholder="VD: Nguyen Van A">
                </div>
                <div class="form-group">
                    <label>Email</label>
                    <input type="email" name="email" value="${sinhVien.email}" placeholder="email@example.com">
                </div>
                <div class="form-group">
                    <label>Lop</label>
                    <input type="text" name="lop" value="${sinhVien.lop}" placeholder="VD: CNTT1">
                </div>
                <div class="form-group">
                ...
                <div class="btn-row">
                    <button type="submit" class="btn btn-primary">${empty sinhVien ? 'Them' : 'Luu thay doi'}</button>
                    <a href="${pageContext.request.contextPath}/staff/sinhvien" class="btn btn-secondary">Huy</a>
                </div>
            </form>
        </div>
    </div>
